...
router.post('/purchase/purchaseitems', authenticateToken, async (req, res) => {
  ...
  mysqlpool.getConnection((err, connection) => {
    ...
    connection.beginTransaction(async (err) => {
      if (err) {
        connection.release();
        return res.status(501).json({ success: false, error: "Transaction error!" });
      }
      ...
      connection.query('SELECT balance FROM wallets WHERE userId = ?', [userId], (err, results) => {
        ...
        if (walletCurrency !== currency) {
          connection.rollback(() => connection.release());
          return res.status(402).json({ success: false, error: `Wallet currency mismatch: expected ${walletCurrency}` });
        }
        
        if (balance < totalCost) {
          connection.rollback(() => connection.release());
          return res.status(403).json({ success: false, error: "Insufficient funds" });
        }
